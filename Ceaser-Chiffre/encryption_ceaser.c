...
void caeser_chiffre_encryption(char *text, const int shift_value)
{
    for (int i = 0; text[i] != '\0'; i++)                                         //Wir interieren durch den Text Buchstabe für Buchstabe
    {
        ...
        {
            text[i] = (char)((text[i] - 'a' + shift_value) % 26 + 'a');           // 'e' ACSII 101 minus 'a' ASCII 97 = 4 + shiftValue von 1 = 5 mod 26 = 5 + 'a' = 'f'ASCII 102
        ...
        {
            text[i] = (char)((text[i] - 'A' + shift_value) % 26 + 'A');            // das gleiche nur für Großbuchstaben
        }
    }
}
