...
int frequencyAnalysis(const char *text)
{
    ...
    // Wir zaehlen fuer jeden Buchstaben die komplementaere Stelle im letter-count Array hoch
    ...
    // Die Haeufigkeit jedes Buchstabens der deutschen Sprache:
    double probability_distribution_german[26] = {0.0651, 0.0189, 0.0306, 0.0508, 0.1740, 0.0166, 0.0301, 0.0476, 0.0755, 0.0027, 0.0121, 0.0344, 0.0253, 0.0978, 0.0251, 0.0079, 0.0002, 0.0700, 0.0727, 0.0615, 0.0435, 0.0067, 0.0189, 0.0003, 0.0004, 0.0113};

    // In diesem Array speichern wir die Differenz der Hauefigkeit jedes Buchstabens in letter_count zu der Hauefigkeit jedes Buchstabens der deutschen Sprache
    ...
    // wir verschieben unsere Verteilung in letter_count 25 mal um 1 und schauen bei welcher Verschiebung die Verteilung am aehnlichsten mit der der deutschen Sprache ist
    for (int i = 0; i < 26; i++)
    {
        for (int j = 0; j < 26; j++)
        {
            // fabs ist aus math.h und gibt den absoluten Differenzwert zurueck, da wir ja keine negativen Werte wollen
