...
char **split_into_subarrays(char *text,int n,const unsigned long int length) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        int j = 0;
        for (int k = i; k < length; k += n) { // i bestimmt den String in den geschrieben wird, j bestimmt den Platz des Characters im string,
            strings[i][j] = text[k];
            j++;
        }
