...
int find_key_size(int *distances, const int length) {
    ...
    // wir berrechnen den groessten, am hauefigst vorkommenden Teiler. Wir erstellen ein array von so vielen Plaetzen die wir braucen und zaehelen am passenden Index hoch wenn das element in den Teiler der Abstaende vorkommt. dann iterieren wir rueckwaerts durchs array und bestimmmen mit if( >= ) den am hauefigst vorkommenden groessten Teiler, bei SChluessellaenge 4 sollte die 4 so ueber der 2 ausgewaehlt werden. trotzdem kann failen, da ein Abstand nicht durch Schluessellaenge teilbar aber durch 2 ==> 2 gibt es mehr als 4
    int kgT = -1;
